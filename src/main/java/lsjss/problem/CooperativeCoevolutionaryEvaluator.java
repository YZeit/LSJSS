...
public class CooperativeCoevolutionaryEvaluator extends Evaluator
{
    ...
    public void setup( final EvolutionState state, final Parameter base )
    {
        ...
        Parameter tempSubpop = new Parameter( Initializer.P_POP ).push( Population.P_SIZE );
        ...
        if (numShuffled > 0) {
            ...
            for (int i = 0; i < state.population.subpops[0].individuals.length; i++)
                for (int k = 0; k < numShuffled; k++) {
                    ...
                    /** Here I add the evaluated individual (combination of both subpops) to the statistics (and its fitness)**/
                    GPIndividual JSSInd = (GPIndividual) inds[0];
                    GPIndividual LSSInd = (GPIndividual) inds[1];
                    CoevolutionStatistics GPStats = (CoevolutionStatistics) state.statistics;
                    GPStats.individualsPerGeneration[state.generation][0][i] = JSSInd.trees[0].child.makeCTree(true,
                            JSSInd.trees[0].printTerminalsAsVariablesInC, JSSInd.trees[0].printTwoArgumentNonterminalsAsOperatorsInC);
                    GPStats.individualsPerGeneration[state.generation][1][i] = LSSInd.trees[0].child.makeCTree(true,
                            LSSInd.trees[0].printTerminalsAsVariablesInC, LSSInd.trees[0].printTwoArgumentNonterminalsAsOperatorsInC);
        ...
        for(int j = 0; j < state.population.subpops.length; j++)
        {
            ...
            for(int i = 0; i < state.population.subpops[j].individuals.length; i++)
            {
                ...
                for(int k = 0; k < eliteIndividuals[j].length; k++)
                {
                    ...
                    GPIndividual LSSInd = (GPIndividual) inds[1];
                    CoevolutionStatistics GPStats = (CoevolutionStatistics) state.statistics;
                    GPStats.individualsPerGeneration[state.generation][0][i+(j*state.population.subpops[j].individuals.length)] = JSSInd.trees[0].child.makeCTree(true,
                            JSSInd.trees[0].printTerminalsAsVariablesInC, JSSInd.trees[0].printTwoArgumentNonterminalsAsOperatorsInC);
                    GPStats.individualsPerGeneration[state.generation][1][i+(j*state.population.subpops[j].individuals.length)] = LSSInd.trees[0].child.makeCTree(true,
                            LSSInd.trees[0].printTerminalsAsVariablesInC, LSSInd.trees[0].printTwoArgumentNonterminalsAsOperatorsInC);
                ...
                for(int k = 0; k < numCurrent; k++)
                {
                    for(int ind = 0; ind < inds.length; ind++)
                    {
                        // define both candidates for both subpopulations and form the individual with its two components
                        if (ind == j) { inds[ind] = individual; updates[ind] = true; } // if the current subpop is equal to the one to be combined
                        else { inds[ind] = produceCurrent(ind, state, 0); updates[ind] = true; } // otherwise randomly select one individual from the other population
                    }
                    prob.evaluate(state,inds,updates, false, subpops, 0);
                    /** Here I add the evaluated individual (combination of both subpops) to the statistics (and its fitness)**/
                    GPIndividual JSSInd = (GPIndividual) inds[0];
